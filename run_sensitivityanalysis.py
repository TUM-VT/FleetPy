...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "sensitivity_study", "scenarios")
        ...
        # vehicle number variation
        #
        # vehicle_num_max = 20
        # for n in range(1, vehicle_num_max + 1, 2):
        #     log_level = "info"
        #     cc = os.path.join(scs_path, "constant_config_ir.csv")
        #     sc = os.path.join(scs_path, f'vehicle_num_var_{n}.csv')
        #     run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=1)
        #     list_results = read_outputs_for_comparison(cc, sc)
        #     all_scenario_assert_dict = {0: {"number users": 88}}
        #     check_assertions(list_results, all_scenario_assert_dict)
        ...
        # vehicle_cap_max = 20
        # for n in range(1, vehicle_cap_max + 1, 2):
        #     log_level = "info"
        #     cc = os.path.join(scs_path, "constant_config_ir.csv")
        #     sc = os.path.join(scs_path, f'vehtype_cap_var_{n}.csv')
        #     run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=1)
        #     list_results = read_outputs_for_comparison(cc, sc)
        #     all_scenario_assert_dict = {0: {"number users": 88}}
        #     check_assertions(list_results, all_scenario_assert_dict)
        ...
        speed_var_max = 20
        for n in range(1, speed_var_max + 1, 2):
            log_level = "info"
            cc = os.path.join(scs_path, f'constant_config_ir_v-{n}.csv')
            sc = os.path.join(scs_path, f'speed_var_{n}.csv')
            run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=1)
            list_results = read_outputs_for_comparison(cc, sc)
            all_scenario_assert_dict = {0: {"number users": 88}}
            check_assertions(list_results, all_scenario_assert_dict)

        # # vehicle range variation

        # vehicle_range_max = 20
        # for n in range(1, vehicle_range_max + 1, 2):
        #     log_level = "info"
        #     cc = os.path.join(scs_path, "constant_config_ir.csv")
        #     sc = os.path.join(scs_path, f'vehtype_range_var_{n}.csv')
        #     run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=1)
        #     list_results = read_outputs_for_comparison(cc, sc)
        #     all_scenario_assert_dict = {0: {"number users": 88}}
        #     check_assertions(list_results, all_scenario_assert_dict)
