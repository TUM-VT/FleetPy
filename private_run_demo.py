...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "demo", "scenarios")  # TODO: change demo to your own study name
        cc = os.path.join(scs_path, "constant_config.csv")

        # Scenario 1
        sc = os.path.join(scs_path, "scenario_config.csv")  # TODO: Either configure multiple scenarios in one scenario config file or create a scenario config file for each scenario
        run_scenarios(cc, sc)

        # # Scenario 2
        # sc = os.path.join(scs_path, "scenario_config_2.csv")  # TODO: Either configure multiple scenarios in one scenario config file or create a scenario config file for each scenario
        # run_scenarios(cc, sc)
