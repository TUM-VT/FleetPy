    ...
    def createNetworkFromNetwokXGraph(self, speed_unit="kmh"):
        ...
        for start_node, end_dict in arcs.items():
            ...
            for end_node, data in end_dict.items():
                ...
                #print(max_v_str)
                #print(type(max_v_str))
                if type(max_v_str) == list:
                    if max_v_str[0] == "none" or max_v_str[0] == "signals":
                        print("no maximal velocity defined on this edge! -> {} km/h considered currently".format(
                            DEFAULT_MAX_V))
                        max_v_str = str(DEFAULT_MAX_V)
                    else:
                        max_max_v = None
                        for x in max_v_str:
                            try:
                ...
                if not max_v_str or max_v_str == "none" or max_v_str == "signals":
...
# MAIN FUNCTION #
#------------------------------------------------------------------------------------------------------------#
def createNetwork(target_network_path, network_name, bbox=None, polygon=None, by_name=None, network_type='drive', osmnG=None, unit_speeds="kmh", from_crs_to_crs = None):
    """ creates network from osmnx
    network_type specified from osmnx (e.g. drive or all)
    either bounding_box (x_max, x_min, y_max, y_min)  or polygon (shapely) has to be given
    ...
    also a osmnx_graph osmnG can be given directly for convertion
    from_crs_to_crs: convert tuple epsg_now, epsg_later
    """
...
if __name__ == "__main__":
    ...
    main_dir = os.path.dirname(os.path.dirname(os.path.dirname(script_dir)))
    network_data_main_dir = os.path.join(main_dir, "data", "networks")
    print(network_data_main_dir)

    # bbox = (48.109435, 48.077543, 11.655696, 11.619060)
    # output_folder = r"C:\Users\ge37ser\Documents\Coding\commonNetworkStructure\networks_git_rep\networks_git\networks\network_data"
    # name = "osmn_test_areaAroundBusLine196_new"
    # createNetwork(r'C:\Users\ge37ser\Documents\Coding\commonNetworkStructure\networks_git_rep\networks_git\networks\network_data', name, bbox=bbox)

    # from createNetworkForEasyRideLine196 import *
    # osm_G = createEasyRideLine196NETWORKX()
    # createNetwork(output_folder, "EasyRideBusLine196_network", osmnG=osm_G)

    #area_by_name = "Manhattan Island, Manhattan, New York City, New York, United States of America"
    # unit_max_speed = "mph"
    # area_by_name = "Manhattan"
    # output_folder = network_data_main_dir
    # name = "Manhattan"
    # createNetwork(output_folder, name, by_name=area_by_name, unit_speeds=unit_max_speed)

    # easyride ap2340
    # bbox = (48.138645, 48.073435, 11.712840, 11.608442)
    # output_folder = network_data_main_dir
    # name = "osmn_munich_ne_er2340_raw"
    # createNetwork(output_folder, name, bbox=bbox)

    # grafing
    # bbox = (48.0607319, 48.0279091, 11.9795774, 11.9212983)
    # output_folder = network_data_main_dir
    # name = "osm_grafing"
    # createNetwork(output_folder, name, bbox=bbox, network_type="all")
    
    # rpp field test 48.169132, 11.551004 | 48.134374, 11.589895
    #bbox = (48.169132, 48.134374, 11.589895, 11.551004)
    #output_folder = network_data_main_dir
    #name = "osm_maxvorstadt_bike"
    #createNetwork(output_folder, name, bbox=bbox, network_type="bike")

    # München Stadtgebiet
    munich_gdf = gpd.read_file("data/networks/munich_network/munich-polygon.geojson")
    polygon = munich_gdf.loc[0, "geometry"]
    output_folder = network_data_main_dir
    name = "munich_city_network"
    createNetwork(output_folder, name, polygon=polygon, network_type="drive")
