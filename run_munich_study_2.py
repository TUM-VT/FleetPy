...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "munich_study", "scenarios")
        # # Base Examples IRS only
        # # ----------------------

        log_level = "info"
        cc = os.path.join(scs_path, "constant_config_munich.csv")
        sc = os.path.join(scs_path, "munich_study_90rqfiles_4vehtypes_4num_2rootpool_study4_2.csv")
        run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=40)
