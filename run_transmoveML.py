...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "transmove", "scenarios")
        
        log_level = "info"
        cc = os.path.join(scs_path, "constant_config_mo_served.csv")
        sc = os.path.join(scs_path, "scenario_config_mo_served.csv")
        n_cpu_per_sim = 1
        run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=n_cpu_per_sim, n_parallel_sim=1)
        
        print('Simulation finished, took', time.time() - t0, 's')
