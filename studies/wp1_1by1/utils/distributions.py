...
class UniformTimeDistribution(RandomDistribution):
    """Fixed-count arrival process on [0, end_time): the number of arrivals is always exactly
    round(expected_requests) (no run-to-run count variation), and arrival times are i.i.d.
    Uniform(0, end_time). Total demand is deterministic given a density/corridor combo -- only the
    arrival ordering/timing is randomized (via rng/seed)."""
    ...
    def sample_count(self, expected_requests, rng):
        # rng is unused (count is fixed, not sampled) -- kept for signature parity with
        # HubScheduleTimeDistribution.sample_count.
        return round(expected_requests)

    def sample(self, rng, direction=None, min_offset=0.0):
        # direction/min_offset are ignored: a uniform arrival process is direction- and
        # travel-time-agnostic. The parameters exist only so all time distributions share one
        # sample() signature (see demand_utils).
...
class HubScheduleTimeDistribution(RandomDistribution):
    ...
    around those events rather than spread uniformly. The total count is fixed (always exactly
    round(expected_requests), see sample_count) -- only which event/offset each request lands on
    is randomized, exactly as in UniformTimeDistribution.
    ...
    plus an optional min_offset floor (see sample()). The offset's sign is directional:
    ...
    With ramp_s <= headway_s (and min_offset=0) the pulses don't overlap. (event, offset) pairs
    landing outside [0, end_time) are rejected and redrawn rather than clamped to the boundary --
    clamping would pile every out-of-range draw onto a single instant (e.g. every to-hub request
    within ramp_s of the very first event collapsing onto rq_time=0), which is a lot more
    artificial spiking than a genuinely truncated first/last pulse."""
    ...
    def sample_count(self, expected_requests, rng):
        # rng is unused (count is fixed, not sampled) -- kept for signature parity with
        # UniformTimeDistribution.sample_count.
        return round(expected_requests)

    def sample(self, rng, direction=None, min_offset=0.0):
        """min_offset: hard floor added to the sampled offset before applying direction's sign --
        used by demand_utils to guarantee a to-hub request is never timed later than the minimum
        physically possible travel time to the hub would allow (an event closer than that is not
        just unlikely to be reached, it's impossible to reach, regardless of service quality)."""
        while True:
            event = self.event_times[rng.integers(0, len(self.event_times))]
            # Right-triangular offset: peak at min_offset, linearly decaying to zero at
            # min_offset + ramp_s.
            offset = min_offset + rng.triangular(0.0, 0.0, self.ramp_s)
            if direction == G_DIR_TO_HUB:
                ...
                # min_offset is expected to be 0 here (no reach-the-hub constraint applies to a
                # traveler already at the hub when the event fires).
                rq_time = event + offset
