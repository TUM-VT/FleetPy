...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "wctr26", "scenarios")

        log_level = "info"
        cc = os.path.join(scs_path, "constant_config_ir.csv")
        sc = os.path.join(scs_path, "s1_2hours_configs.csv")
        run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=12)

        log_level = "info"
        cc = os.path.join(scs_path, "constant_config_ir.csv")
        sc = os.path.join(scs_path, "s1_configs.csv")
        run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=12)
