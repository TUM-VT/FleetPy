...
class GNNAlonsoMoraAssignment(AlonsoMoraAssignmentOriginal):
    ...
    GNN_MODEL_PATH = 'notebooks/data/models/best_model.pt'
    ...
    def _predict_xgboost(self, data):
        ...
        # Process features
        merged, _ = self._data_processor.process_single_timestep(
            data, edge_type='vr_graph', timestep=self.sim_time)
    ...
    def _create_hetero_graph(self, data):
        ...
        graph['request'].x = torch.FloatTensor(
            req_features.drop(['timestep'], axis=1).values)
        graph['vehicle'].x = torch.FloatTensor(
            veh_features.drop(['timestep'], axis=1).values)

        # Add edge features
        vr_edges = pd.DataFrame(data['vr_graph'])
        edge_index = torch.tensor([[int(s), int(t)] for s, t in zip(
            vr_edges['source'], vr_edges['target'])], dtype=torch.long).t()
        edge_attr = torch.FloatTensor(vr_edges.drop(
            ['source', 'target', 'label'], axis=1).values)
    ...
    def _update_connection_maps(self, pruned_edges):
        ...
        print(
            f"Kept {n_valid} valid connections out of {n_total} predicted connections")

    def _is_RR_pred_compatible(self, vid: int, rid: int, edge_type: str) -> float:
