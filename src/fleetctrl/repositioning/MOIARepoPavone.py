...
class MOIARepoPavone(RepositionBase):
    ...
    def determine_and_create_repositioning_plans(self, sim_time, lock=True):
        ...
        for zone_id in list_zones:
            if self.zone_system.get_random_centroid_node(zone_id) < 0:
                ...
                v_i_d_dict[zone_id] = zone_call
                n_called += zone_call
        while n_called > total_excess_vehicles:
            max_zone, max_val = max(v_i_d_dict.items(), key = lambda x:x[1])
            if max_val <= 0:
                break
            v_i_d_dict[max_zone] -= 1
            n_called -= 1
        # rebalancing policy -> divide excess vehicles evenly among all zones
        # v_i_d_dict = {}
        # total_excess_vehicles = sum(list(v_i_e_dict.values()))
        # if total_excess_vehicles > 0 and len(list_zones) > 0:
        #     avg_excess_vehicles_per_zone = int(total_excess_vehicles / len(list_zones))
        #     # avg_excess_vehicles_per_zone = float(total_excess_vehicles)/len(list_zones)# int(total_excess_vehicles / len(list_zones))
        #     # if total_excess_vehicles > 0 and avg_excess_vehicles_per_zone == 0:
        #     #     avg_excess_vehicles_per_zone = 1
        # else:
        ...
        # total_access_vehicles = int(sum(list(v_i_d_dict.values())))
        # LOG.info("v_i_e_dict {}".format(v_i_e_dict))
        # LOG.info("v_i_d_dict {}".format(v_i_d_dict))
        # LOG.info("total excess {} total access {}".format(total_excess_vehicles, total_access_vehicles))
        #exit()
