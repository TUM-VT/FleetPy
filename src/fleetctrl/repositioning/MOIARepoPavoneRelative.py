...
class MOIARepoPavoneRelative(RepositioningBase):
    ...
    adoption in this class:
    forecast no absolute values but relative values
    these values are rescaled according to the number of active vehicles
    vehicles are distributed to maintain this distribution
    """
    ...
    def __init__(self, fleetctrl, operator_attributes, dir_names, solver="Gurobi"):
        ...
        self.frac_rebalancing_vehicles = operator_attributes.get(G_OP_REPO_REL_FRAC, 1.0)
    ...
    def determine_and_create_repositioning_plans(self, sim_time, lock=True):
        ...
        #     list_zones.append(-1)
        all_vehicles = sum(vehicles_repo_to_zone.values()) + total_idle_vehicles
        all_fc = sum(demand_fc_dict.values())   # no supply fc
        # rescale demand fc
        for k, v in demand_fc_dict.items():
            demand_fc_dict[k] = v*all_vehicles/all_fc * self.frac_rebalancing_vehicles
        o_d_diff_list = []
        omega_dict = {}
        for zone_id in list_zones:
            ...
            omega_dict[zone_id] = number_idle + o_d_diff
            zone_dict[zone_id] = zone_counter
            zone_counter += 1
        # my rebalancing policy
        v_i_d_dict = {}
        total_excess_vehicles = int(sum(list(v_i_e_dict.values())))
        for i in range(int(total_excess_vehicles * self.frac_rebalancing_vehicles)):
            o_d_diff_list.sort(key=lambda x:x[1])
            zone_id, val = o_d_diff_list[0]
            try:
                v_i_d_dict[zone_id] += 1
            except:
                v_i_d_dict[zone_id] = 1
            o_d_diff_list[0] = (zone_id, val + 1)
        ...
        # with open( os.path.join(self.fleetctrl.dir_names[G_DIR_OUTPUT], f"reloc_{sim_time}.csv"), "w") as f:
        #     lines = []
        #     lines.append("omega_dict")
        #     for zone_id, value in omega_dict.items():
        #         lines.append(f"{zone_id},{value}")
        #     lines.append("reloc")
        #     for ozone, dzone in od_reposition_trips:
        #         lines.append(f"{ozone},{dzone}")
        #     lines.append("vie")
        #     for z, v in v_i_e_dict.items():
        #         lines.append(f"{z},{v}")
        #     lines.append("vid")
        #     for z, v in v_i_d_dict.items():
        #         lines.append(f"{z},{v}")
        #     f.write("\n".join(lines))
        # print("")
