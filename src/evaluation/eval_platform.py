...
def add_user_trip_occupancies_to_stats(output_dir, evaluation_start_time = None, evaluation_end_time = None, dir_names_in = {}):
    ...
    for op_vid, veh_df in op_stats_all.groupby([G_V_OP_ID, G_V_VID]):
        for start_time, end_time, distance, ob_str, boarding_str, alight_str, occ in \
                    zip(veh_df[G_VR_LEG_START_TIME].values, veh_df[G_VR_LEG_END_TIME].values, veh_df[G_VR_LEG_DISTANCE].values, veh_df[G_VR_OB_RID].values, veh_df[G_VR_BOARDING_RID].values, veh_df[G_VR_ALIGHTING_RID].values, veh_df[G_VR_NR_PAX].values):
            ob_rids = []
            boarding_rids = []
            alight_rids = []
            if ob_str == ob_str:
                ob_rids = [int(x) for x in ob_str.split(";")]
            if boarding_str == boarding_str:
                boarding_rids = [int(x) for x in boarding_str.split(";")]
            if alight_str == alight_str:
                alight_rids = [int(x) for x in alight_str.split(";")]
            rids = set(ob_rids + boarding_rids + alight_rids)
            for rid in rids:
                if user_to_occ_stats.get(rid) is None:
                    user_to_occ_stats[rid] = {}
                try:
                    user_to_occ_stats[rid][f"time occ {occ}"] += end_time - start_time
                except KeyError:
                    user_to_occ_stats[rid][f"time occ {occ}"] = end_time - start_time

    user_occ_stats_df = pd.DataFrame.from_dict(user_to_occ_stats, orient="index")
    user_stats.set_index(G_RQ_ID, inplace=True)
    merged_users = pd.concat([user_stats, user_occ_stats_df], axis=1)
    merged_users.index.name = G_RQ_ID
    merged_users.to_csv(os.path.join(output_dir, "1_user-stats.csv"))


def eval_platform_scenario(output_dir, evaluation_start_time = None, evaluation_end_time = None, print_comments=False, dir_names_in = {}):
    ...
    for op_id in range(scenario_parameters[G_NR_OPERATORS] + 1):
        ...
        if op_id >= 0:  #AMoD
            ...
            try:
                ...
                op_stats["VRL_end_sim_end_time"] = np.minimum(op_stats[G_VR_LEG_END_TIME], sim_end_time)
                op_stats["VRL_start_sim_end_time"] = np.minimum(op_stats[G_VR_LEG_START_TIME], sim_end_time)
                utilized_veh_df = op_stats[(op_stats["status"] != VRL_STATES.OUT_OF_SERVICE.display_name) & (op_stats["status"] != VRL_STATES.CHARGING.display_name)]
                utilization_time = utilized_veh_df["VRL_end_sim_end_time"].sum() - utilized_veh_df["VRL_start_sim_end_time"].sum()
                unutilized_veh_df = op_stats[(op_stats["status"] == VRL_STATES.OUT_OF_SERVICE.display_name) | (op_stats["status"] == VRL_STATES.CHARGING.display_name)]
            ...
            try:
                ...
                op_stats["weighted_ob_rq"] = 0
                op_stats["weighted_ob_pax"] = 0
            ...
            for vid, vid_vtype_row in op_veh_types.iterrows():
                ...
                op_vid_vehicle_df = op_stats[op_stats[G_V_VID] == vid]
                ...
                all_vid_dict[vid] = {"type":vtype_data[G_VTYPE_NAME], "total km":veh_km, "total kWh": veh_kWh,
                                    ...
                                    "total variable costs": veh_var_costs, "revenue" : revenue}
                if "driver_id" in op_vid_vehicle_df.columns:
                    driver_id = op_vid_vehicle_df["driver_id"].unique()
                    all_vid_dict[vid]["driver_id"] = driver_id[0]
                occ_distance = {}
                for occ, occ_df in op_vid_vehicle_df.groupby(G_VR_NR_PAX):
                    d = occ_df[G_VR_LEG_DISTANCE].sum()/1000.0
                    if d > 0:
                        occ_distance[f"km occ {occ}"] = occ_df[G_VR_LEG_DISTANCE].sum()/1000.0
                all_vid_dict[vid].update(occ_distance)
        ...
        result_dict["customer in vehicle distance"] = avg_in_vehicle_distance(op_stats)
        result_dict["shared rides [%]"] = shared_rides(op_stats)
    ...
    for f in os.listdir(path):
        ...
        if os.path.isdir(sc_path):
            if os.path.isfile(os.path.join(sc_path, "1_user-stats.csv")):
                eval_platform_scenario(sc_path, evaluation_start_time=evaluation_start_time, evaluation_end_time=evaluation_end_time, print_comments=print_comments)
...
if __name__ == "__main__":
    ...
    sc = r"C:\Users\ge37ser\Documents\Coding\TUM_VT_FleetSimulation\tum-vt-fleet-simulation\FleetPy\studies\maasim_fleetpy_trb24\results\test"
    add_user_trip_occupancies_to_stats(sc)
    #eval_platform_scenario(sc, print_comments=True)