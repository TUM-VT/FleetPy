...
class Trainer:
    ...
    def train_epoch(self, model, optimizer, criterion):
        """Train for one epoch with improved monitoring and debugging"""
        model.train()
        total_loss = 0
        all_preds = []
        all_probs = []
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            optimizer.zero_grad()
            # print(f"[DEBUG] Training batch {batch_idx + 1}/{len(self.train_loader)}")
            # # Debug: Print input feature stats
            # for node_type, x in batch.x_dict.items():
            #     print(f"[DEBUG] Batch {batch_idx} Node type '{node_type}' features: mean={x.float().mean().item():.4f}, std={x.float().std().item():.4f}, min={x.float().min().item():.4f}, max={x.float().max().item():.4f}")
            # for edge_type, edge_attr in batch.edge_attr_dict.items():
            #     print(f"[DEBUG] Batch {batch_idx} Edge type '{edge_type}' edge_attr: mean={edge_attr.float().mean().item():.4f}, std={edge_attr.float().std().item():.4f}, min={edge_attr.float().min().item():.4f}, max={edge_attr.float().max().item():.4f}")
            ...
            # print(f"[DEBUG] Batch {batch_idx} Target: mean={target.mean().item():.4f}, std={target.std().item():.4f}, min={target.min().item():.4f}, max={target.max().item():.4f}, bincount={torch.bincount(target.long()).tolist()}")
            ...
            # Debug: Print logits stats before activation
            # print(f"[DEBUG] Batch {batch_idx} Logits: mean={logits.mean().item():.4f}, std={logits.std().item():.4f}, min={logits.min().item():.4f}, max={logits.max().item():.4f}")
            ...
            # Debug: Print gradient stats for model parameters
            # for name, param in model.named_parameters():
            #     if param.grad is not None:
            #         print(f"[DEBUG] Grad {name}: mean={param.grad.mean().item():.6f}, std={param.grad.std().item():.6f}, min={param.grad.min().item():.6f}, max={param.grad.max().item():.6f}")
            #     else:
            ...
            with torch.no_grad():
                ...
                # print(f"[DEBUG] Batch {batch_idx} Probs: mean={probs.mean().item():.4f}, std={probs.std().item():.4f}, min={probs.min().item():.4f}, max={probs.max().item():.4f}")
